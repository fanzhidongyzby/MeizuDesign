	...
	if(cache && div) {
		...
		x.get(href + '&inajax=1&ajaxmenuid='+ctrlid+'_menu', function(s) {
			...
			if(!evaled && (typeof ajaxerror == 'undefined' || !ajaxerror)) {
				...
				showMenu(ctrlid, x.etype == 'click', 0, x.duration, x.timeout, 0, ctrlid, 500, 1);
...
function ajaxmenu2(e, ctrlid, timeout, func, cache, duration, ismenu, divclass, optionclass)
{
   ajaxmenu(e, ctrlid, timeout, func, cache, duration, ismenu, divclass, optionclass);
   setTimeout('function reload(){ window.location.href=window.location.href;};reload();',1000);
	...
	if(action == 'open') {
		...
		if(!$('pmlayer')) {
			...
			if (param == 'mzguide'){
				...
				div.style.left = ((clientWidth - 500) / 2) + 'px';
				$('append_parent').appendChild(div);
				$('pmlayer').innerHTML = '<div style="width: 475px; background: #666666; margin: 5px auto; text-align: left">' +
					'<div style="width: 475px; height: 317px; padding: 1px; background: #FFFFFF; border: 1px solid #7597B8; position: relative; left: -6px; top: -3px">' +
					'<div onmouseover="pmwindrag(this)" style="cursor: move; position: relative; left: 0px; top: 0px; width: 475px; height: 30px; margin-bottom: -30px;"></div>' +
					'<a href="###" onclick="pmwin(\'close\')"><img style="position: absolute; right: 20px; top: 15px" src="' + IMGDIR +'/close.gif" title="关闭" /></a>' +
					'<div id="pmwinmask" style="margin-top: 30px; position: absolute; width: 100%; height: 100%; display: none"></div><iframe id="pmframe" name="pmframe" style="width:475px;height:317px" allowTransparency="true" frameborder="0"></iframe></div></div>';
			}else{
		...
		if (param == 'mzguide'){
			$('pmlayer').style.top = ((clientHeight - 320) / 2 + scrollTop) + 'px';
		...
		}else if (param == 'mzguide'){
			pmframe.location = 'mzguide.php';
		}else {
